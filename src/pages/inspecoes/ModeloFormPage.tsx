...
const ModeloFormPage = () => {
  ...
  useEffect(() => {
    if (!loadingSetores && !setoresError && !isEdit && !setor && setoresDisponiveis.length > 0) {
      setSetor(setoresDisponiveis[0]);
    }
  }, [loadingSetores, setoresError, isEdit, setor, setoresDisponiveis]);

  useEffect(() => {
    if (setoresError) return;
    if (isEdit) {
      void (async () => {
        try {
          const modelo = await getModeloInspecao(id!);
          if (!loadingSetores && !hasSetorPermitido(setoresPermitidos, modelo.setor)) {
  ...
  }, [id, isEdit, loadingSetores, setoresError, navigate, setoresPermitidos, toast]);
