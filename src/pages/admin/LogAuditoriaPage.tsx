...
const LogAuditoriaPage = () => {
  ...
  const filteredTrans = transitionLogs.filter(
    (l) =>
      l.usuario.toLowerCase().includes(transSearch.toLowerCase()) ||
      l.osId.toLowerCase().includes(transSearch.toLowerCase()) ||
      l.planta.toLowerCase().includes(transSearch.toLowerCase()) ||
      (l.motivo || "").toLowerCase().includes(transSearch.toLowerCase())
  );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs defaultValue="geral">
        <TabsList>
          ...
          <TabsTrigger value="transicoes">Transições de OS</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="transicoes" className="space-y-4">
          <div className="relative max-w-sm">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input placeholder="Buscar por OS, usuário, planta, motivo..." value={transSearch} onChange={(e) => setTransSearch(e.target.value)} className="pl-9" />
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Usuário</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Perfil/Papel</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Planta</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">De → Para</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">Motivo</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTrans.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-6 text-muted-foreground text-sm">Nenhuma transição registrada</td></tr>
                  ) : filteredTrans.map((l) => (
                    <tr key={l.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground whitespace-nowrap">{l.timestamp.replace("T", " ").slice(0, 16)}</td>
                      <td className="px-4 py-3 text-xs font-mono text-primary">{l.osId}</td>
                      <td className="px-4 py-3 font-medium text-foreground text-xs">{l.usuario}</td>
                      <td className="px-4 py-3 text-xs">{l.perfil} / {l.papel}</td>
                      <td className="px-4 py-3 text-xs">{l.planta}</td>
                      <td className="px-4 py-3 text-xs">
                        <Badge variant="outline" className="text-[10px] mr-1">{OS_STATUS_LABELS[l.oldStatus]}</Badge>
                        <span className="text-muted-foreground">→</span>
                        <Badge variant="outline" className="text-[10px] ml-1">{OS_STATUS_LABELS[l.newStatus]}</Badge>
                      </td>
                      <td className="px-4 py-3 text-xs text-muted-foreground max-w-[200px] truncate">{l.motivo || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
