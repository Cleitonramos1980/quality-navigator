...
const AcessoDetalhePage = () => {
  ...
  const [selfieInvalida, setSelfieInvalida] = useState(false);

  useEffect(() => {
    if (!id) return;
    getAcessoById(id)
      .then((a) => setAcesso(a || null))
      .catch((error) => {
        setAcesso(null);
        const message = error instanceof Error ? error.message : "Falha ao carregar acesso.";
        toast({ title: "Erro ao carregar acesso", description: message, variant: "destructive" });
      });
    getTimelinePortaria(id)
      .then(setTimeline)
      .catch((error) => {
        const message = error instanceof Error ? error.message : "Falha ao carregar timeline do acesso.";
        toast({ title: "Erro ao carregar timeline", description: message, variant: "destructive" });
      });
  }, [id, toast]);

  useEffect(() => {
    setSelfieInvalida(false);
  }, [acesso?.selfieUrl]);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{acesso.tipo} â€” {acesso.nome}</p>
        </div>
        <div className="flex gap-2">
          ...
          {["ENTRADA_REGISTRADA", "EM_PERMANENCIA"].includes(acesso.status) && (
            <Button size="sm">Registrar SaÃ­da</Button>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <TraceabilityCard
            ...
            dados={[
              ...
              { label: "ResponsÃ¡vel Interno", valor: acesso.responsavelInterno },
              { label: "Placa", valor: acesso.placa },
              { label: "Tipo VeÃ­culo", valor: acesso.tipoVeiculo },
              { label: "HorÃ¡rio Previsto", valor: new Date(acesso.horarioPrevisto).toLocaleString("pt-BR") },
              { label: "HorÃ¡rio Real", valor: acesso.horarioReal ? new Date(acesso.horarioReal).toLocaleString("pt-BR") : undefined },
              { label: "SaÃ­da", valor: acesso.horarioSaida ? new Date(acesso.horarioSaida).toLocaleString("pt-BR") : undefined },
        ...
        <div className="space-y-6">
          {acesso.selfieUrl && (
            <div className="glass-card rounded-lg p-4">
              <h3 className="mb-3 text-sm font-semibold text-foreground">Selfie / IdentificaÃ§Ã£o</h3>
              <div className="aspect-square rounded-lg bg-muted flex items-center justify-center overflow-hidden">
                {selfieInvalida ? (
                  <p className="px-4 text-center text-xs text-muted-foreground">
                    Selfie indisponivel neste cadastro. Solicite novo envio da foto.
                  </p>
                ) : (
                  <img
                    ...
                    onError={() => setSelfieInvalida(true)}
                  />
                )}
              </div>
              <p className="mt-2 text-[10px] text-muted-foreground flex items-center gap-1">
                <Shield className="h-3 w-3" /> Dado sensÃ­vel â€” LGPD. RetenÃ§Ã£o: 90 dias.
              </p>
            </div>
          )}
          ...
          <div className="glass-card rounded-lg p-4">
            ...
            <dl className="space-y-2 text-xs">
              ...
              <div><dt className="text-muted-foreground">Ãšltima AtualizaÃ§Ã£o</dt><dd>{new Date(acesso.ultimaAtualizacao).toLocaleString("pt-BR")}</dd></div>
            </dl>
          </div>
